...
//
// Config resolution (in order): explicit inputs.config -> config.json -> env vars.
...
function loadConfig(override = {}) {
  ...
  return {
    Z_MINT: override.Z_MINT || file.Z_MINT || process.env.Z_MINT,
    Z_THRESHOLD: Number(override.Z_THRESHOLD ?? file.Z_THRESHOLD ?? process.env.Z_THRESHOLD),
    HELIUS_RPC_URL: override.HELIUS_RPC_URL || process.env.HELIUS_RPC_URL,
    SECOND_RPC_URL: override.SECOND_RPC_URL || process.env.SECOND_RPC_URL,
    GATE_SIGNING_SECRET_KEY: override.GATE_SIGNING_SECRET_KEY || process.env.GATE_SIGNING_SECRET_KEY,
  };
}
...
async function signVerdict(verdict, secretKeyBase58) {
  ...
  try {
    ...
    // tweetnacl/bs58 not installed in this context — return unsigned, caller decides.
    return "";
  }
}
